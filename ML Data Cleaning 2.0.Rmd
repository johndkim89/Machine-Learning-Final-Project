...
```{r}
new_pp <- dplyr::select (new_pp, HHINC, HTOTVAL, A_SEX, A_AGE, PRCITSHP, PRDTRACE, PENATVTY, PEFNTVTY, PEMNTVTY, PEPAR1TYP, PEPAR2TYP, H_TENURE, A_MARITL, P_STAT, PEAFEVER, PEHSPNON, PEINUSYR, PRDASIAN, PRDTHSP, A_MJIND, A_MJOCC, A_HRS1, A_GRSWK, PEDISDRS, PEDISEAR, PEDISOUT, PEDISPHY, PEDISREM, GESTFIPS, state, state_full )
```

#Data Cleaning
```{r}
#HTOTVAL (Remove 0）
new_pp1 <- subset(new_pp, !(HTOTVAL==0))
range(new_pp1$HTOTVAL)
range(new_pp1$HHINC)
...
new_pp1 %>% group_by(GESTFIPS) %>% summarise(count = n())
